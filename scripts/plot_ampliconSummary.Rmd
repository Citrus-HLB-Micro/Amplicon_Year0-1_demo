...
```{r}
metatable = file.path(knitr::opts_chunk$get("root.dir"),"sample_info.tsv")
...
```{r}
otutable <- file.path(knitr::opts_chunk$get("root.dir"), "ECDRE_LR_Y0_1_ITS.otu_table.txt")
...
```{r}
taxin = file.path(knitr::opts_chunk$get("root.dir"),"ECDRE_LR_Y0_1_ITS.cluster.taxonomy.fix.txt")
...
```{r warning=FALSE}
treefile = file.path(knitr::opts_chunk$get("root.dir"),"ECDRE_LR_Y0_1_ITS.cluster.tree.phy")
...
```{r fig.align='center'}
plot_richness(physeq.prune.rarefy, x = "Tissue_type", color = "Tissue_type", measures = c("Shannon")) + 
  geom_boxplot() +
...
#### make some phyloseq plots prettier wth microshades
```{r fig.align='center'}
...
color_objs_TR <- create_color_dfs(mdf_prep,
                                  selected_groups = c("Mucoromycota", "Glomeromycota","Ascomycota", "Basidiomycota") , cvd = TRUE)
...
```{r}
plot_ordination(physeq.prune.rarefy, physeq.prune.rarefy.ps.ord, type = "samples", color = "Tissue_type",shape="Location")  + ggtitle("Fungi Beta Diversity (PCoA) by Tissue Type / Location")
```
...
Save plot
```{r}
pdf("Figures/taxcom_name.pdf", width = 6, height = 3.8)
print(taxcom_treatment)
dev.off()
```
...
```{r}
physeq.prune.rarefy.plot.richness = plot_richness(physeq.prune.rarefy, x="Treatment", color=("Tissue_type"), measures=c("Observed")) + geom_boxplot(lwd=0.5) + ggtitle("Alpha Diversity by Tissue_type") + stat_compare_means(method = "t.test", label.x.npc = c("right")) + theme_bw() + scale_colour_brewer(palette="Set1") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
